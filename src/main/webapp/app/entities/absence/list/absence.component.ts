...
import { EntityArrayResponseType, AbsenceService } from '../service/absence.service';
...
export class AbsenceComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAbsence): number => this.absenceService.getAbsenceIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.absences = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IAbsence[] | null): IAbsence[] {
    const absencesNew = this.absences ?? [];
    if (data) {
      for (const d of data) {
        if (absencesNew.map(op => op.id).indexOf(d.id) === -1) {
          absencesNew.push(d);
        }
      }
    }
