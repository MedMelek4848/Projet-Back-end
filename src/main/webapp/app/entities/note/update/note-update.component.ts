...
export class NoteUpdateComponent implements OnInit {
  ...
  protected updateForm(note: INote): void {
    ...
    this.elevesSharedCollection = this.eleveService.addEleveToCollectionIfMissing<IEleve>(this.elevesSharedCollection, note.matriculeEleve);
    this.examenSharedCollection = this.examenService.addExamenToCollectionIfMissing<IExamen>(this.examenSharedCollection, note.nomExamen);
  }
  ...
  protected loadRelationshipsOptions(): void {
    this.eleveService
      ...
      .pipe(map((eleves: IEleve[]) => this.eleveService.addEleveToCollectionIfMissing<IEleve>(eleves, this.note?.matriculeEleve)))
      .subscribe((eleves: IEleve[]) => (this.elevesSharedCollection = eleves));

    this.examenService
      .query()
      .pipe(map((res: HttpResponse<IExamen[]>) => res.body ?? []))
      .pipe(map((examen: IExamen[]) => this.examenService.addExamenToCollectionIfMissing<IExamen>(examen, this.note?.nomExamen)))
