...
export class ProfesseurComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.professeurs = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IProfesseur[] | null): IProfesseur[] {
    const professeursNew = this.professeurs ?? [];
    if (data) {
      for (const d of data) {
        if (professeursNew.map(op => op.id).indexOf(d.id) === -1) {
          professeursNew.push(d);
        }
      }
    }
    return professeursNew;
  }
