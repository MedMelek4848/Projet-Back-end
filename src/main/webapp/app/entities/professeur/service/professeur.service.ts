...
type RestOf<T extends IProfesseur | NewProfesseur> = Omit<T, 'dateContrat'> & {
...
export class ProfesseurService {
  ...
  protected convertDateFromClient<T extends IProfesseur | NewProfesseur | PartialUpdateProfesseur>(professeur: T): RestOf<T> {
    return {
      ...professeur,
      dateContrat: professeur.dateContrat?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProfesseur: RestProfesseur): IProfesseur {
    return {
      ...restProfesseur,
      dateContrat: restProfesseur.dateContrat ? dayjs(restProfesseur.dateContrat) : undefined,
    };
  }
