...
export class ExamenUpdateComponent implements OnInit {
  ...
  protected loadRelationshipsOptions(): void {
    this.matiereService
      ...
      .pipe(map((matieres: IMatiere[]) => this.matiereService.addMatiereToCollectionIfMissing<IMatiere>(matieres, this.examen?.nomMatiere)))
      .subscribe((matieres: IMatiere[]) => (this.matieresSharedCollection = matieres));

    this.classeService
      .query()
      .pipe(map((res: HttpResponse<IClasse[]>) => res.body ?? []))
      .pipe(map((classes: IClasse[]) => this.classeService.addClasseToCollectionIfMissing<IClasse>(classes, this.examen?.nomClasse)))
