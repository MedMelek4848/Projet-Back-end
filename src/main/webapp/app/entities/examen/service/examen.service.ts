...
type RestOf<T extends IExamen | NewExamen> = Omit<T, 'dateExamen'> & {
...
export class ExamenService {
  ...
  protected convertDateFromClient<T extends IExamen | NewExamen | PartialUpdateExamen>(examen: T): RestOf<T> {
    return {
      ...examen,
      dateExamen: examen.dateExamen?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restExamen: RestExamen): IExamen {
    return {
      ...restExamen,
      dateExamen: restExamen.dateExamen ? dayjs(restExamen.dateExamen) : undefined,
    };
  }
