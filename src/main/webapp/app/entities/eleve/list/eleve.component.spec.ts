...
describe('Eleve Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'eleve', component: EleveComponent }]), HttpClientTestingModule],
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.eleves?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to eleveService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getEleveIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getEleveIdentifier).toHaveBeenCalledWith(entity);
