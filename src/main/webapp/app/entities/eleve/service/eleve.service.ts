...
type RestOf<T extends IEleve | NewEleve> = Omit<T, 'dateDeNaissance'> & {
...
export class EleveService {
  ...
  protected convertDateFromClient<T extends IEleve | NewEleve | PartialUpdateEleve>(eleve: T): RestOf<T> {
    return {
      ...eleve,
      dateDeNaissance: eleve.dateDeNaissance?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEleve: RestEleve): IEleve {
    return {
      ...restEleve,
      dateDeNaissance: restEleve.dateDeNaissance ? dayjs(restEleve.dateDeNaissance) : undefined,
    };
  }
