...
/**
 * REST controller for managing {@link com.nourelmaaref.master.domain.Examen}.
 */
@RestController
@RequestMapping("/api")
public class ExamenResource {
    ...
    public ResponseEntity<List<ExamenDTO>> getAllExamen(
        ...
        Page<ExamenDTO> page;
        if (eagerload) {
            page = examenService.findAllWithEagerRelationships(pageable);
        } else {
            page = examenService.findAll(pageable);
        }
