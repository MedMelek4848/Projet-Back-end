...
public interface AbsenceRepository extends JpaRepository<Absence, Long> {
    ...
    @Query(
        value = "select distinct absence from Absence absence left join fetch absence.matriculeEleve left join fetch absence.nomMatiere",
    ...
    @Query("select distinct absence from Absence absence left join fetch absence.matriculeEleve left join fetch absence.nomMatiere")
    List<Absence> findAllWithToOneRelationships();

    @Query(
        "select absence from Absence absence left join fetch absence.matriculeEleve left join fetch absence.nomMatiere where absence.id =:id"
    )
